...
class NNLSaveGraber(SaveBaslerCameras):
    ...
    def handle_img(self,image,camera_id,count):
        ...
        light,dark,has_steel,edge = process_img(image, camera_id)
        info = dict(
        ...
        light_folder = join(self.save_folder,"light")
        dark_folder = join(self.save_folder,"dark")
        if not os.path.exists(light_folder):
            os.makedirs(light_folder)
        if not os.path.exists(dark_folder):
            os.makedirs(dark_folder)

        light_name = join(save_folder,"light","{}_{:0>8d}.{}".format(self.begin_id+camera_id,count,self.save_format))
        dark_name = join(save_folder,"dark","{}_{:0>8d}.{}".format(self.begin_id+camera_id,count,self.save_format))

        # share_name = join(self.src_img_share_url,day_info,trigger_info,"light",
        ...
        info["imgUrl"] = join(day_info,trigger_info,"light","{}_{:0>8d}.{}".format(self.begin_id+camera_id,count,self.save_format))
    ...
    def send_server(self, info):
        ...
        try:
            ...
            req = requests.post("{}/back/upload/img".format(self.server_url), data=values_json, headers=headers)
        except Exception as e:
